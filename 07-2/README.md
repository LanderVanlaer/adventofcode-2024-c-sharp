...
operator (`<span title="I think you mean &quot;.&quot;.">||</span>`) combines the digits from its left and right inputs
into a single number. For example, `12 || 345` would become `12345`. All operators are still evaluated left-to-right.
...
now be made by also using `||`) produces the new _total calibration result_ of _`11387`_.
